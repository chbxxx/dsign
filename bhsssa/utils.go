...
**/
func random(curveType CurveType) *big.Int {
...
**/
func evaluatePolynomial(polynomial []*big.Int, value *big.Int, curveType CurveType) *big.Int {
...
**/
func modInverse(number *big.Int, curveType CurveType) *big.Int {
	prime := GetPrime(curveType)
...
func KeyCombine(p []string, label string, shares []ShareXY, curveType CurveType) *big.Int {
	//shareSum := shares[0]
	...
	num, deno := CalBs(p, label, curveType)
	result := CalFinal(shareSum, num, deno, curveType)
	return result
}
