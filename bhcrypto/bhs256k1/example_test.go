...
func Example_encryptMessage() {
	...
	pubKey, err := bhs256k1.ParsePubKey(pubKeyBytes, bhs256k1.S256())
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	ciphertext, err := bhs256k1.Encrypt(pubKey, []byte(message))
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	privKey, _ := bhs256k1.PrivKeyFromBytes(bhs256k1.S256(), pkBytes)

	// Try decrypting and verify if it's the same message.
	plaintext, err := bhs256k1.Decrypt(privKey, ciphertext)
	if err != nil {
		fmt.Println(err)
		return
	}
